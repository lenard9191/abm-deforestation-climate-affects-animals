...
to deforest
  let num-patches-to-deforest count patches * deforestation-rate  ;
  let deforested-patches n-of num-patches-to-deforest patches  ;
  ask deforested-patches [
    set alive false
    set pcolor brown  ; Set color to black to indicate deforested area
  ]
end

to grow-grass-start
  let num-patches-to-cover count patches * 0.5  ;
  let covered-patches n-of num-patches-to-cover patches  ;
  ask covered-patches [
    if pcolor = black [
      set pcolor green
    ]
...
to eat-grass  ;; rabbit procedure
  ...
  if pcolor = green [
    set pcolor black
    let adjusted-grass-energy grass-energy - (climate-change * grass-energy)  ;
    set energy energy + adjusted-grass-energy
  ]
end
...
to move-predator  ;; predator procedure
  rt random 50
  lt random 50
  fd 1
...
to eat-rabbit  ;; predator procedure
  let target min-one-of rabbits-here [distance myself]
  if target != nobody [
    ask target [
      die
    ]
...
to reproduce-predator  ;; predator procedure
  if hunger > birth-threshold * 2
    [ set hunger hunger / 2
      hatch 1 [ fd 1 ] ]
end

to death-predator  ;; predator procedure
  if hunger < 0 [ die ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"pred" 1.0 0 -7500403 true "" "plot (count predators)\n"
